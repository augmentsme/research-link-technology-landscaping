...
class DataExploration:
    ...
    def _prepare_display_dataframe(self, display_keywords: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in display_df.columns:
            if col not in column_order and col != 'grants':
                column_order.append(col)
        
        show_grants_detail = st.checkbox(
            "Show grant IDs details", 
            value=False,
            help="Include the detailed list of grant IDs for each keyword"
        )
        
        if show_grants_detail:
