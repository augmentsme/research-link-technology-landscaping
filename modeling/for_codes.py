"""
FOR Codes Data Processing

This module handles the complete processing pipeline for Australian and New Zealand 
Standard Research Classification (ANZSRC) Field of Research (FOR) codes.
It includes both cleaning/structuring from Excel files and processing functions.
"""
...
from config import DATA_DIR, FOR_CODES_CLEANED_PATH, ROOT_DIR
...
def clean_for_codes(output_path=None):
...
def process_for_codes():
    """
    Process FOR (Fields of Research) codes data.
    
    Returns:
        tuple: (divisions_count, groups_count) or (0, 0) if processing failed
    """
    print("Processing FOR codes data...")
...
if __name__ == "__main__":
    print("=== FOR Codes Processing ===\n")
    
    divisions, groups = process_for_codes()
    
    print(f"FOR codes processed: {divisions} divisions, {groups} groups")
    print("\n" + "="*50)
    print("🎉 FOR codes processing completed!")
    print("="*50)
