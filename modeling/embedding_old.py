"""
Embedding generation module for keyword vectorization using the unified ChromaDB manager.

This module provides functionality to create embeddings for keywords and store them in
ChromaDB vector database using the UnifiedChromaDBManager. It delegates embedding generation
to ChromaDB using a custom OpenAI-compatible embedding function that connects to vLLM servers.

Key features:
- Uses UnifiedChromaDBManager for consistent data management
...
class EmbeddingManager:
    ...
    def __init__(self, vllm_base_url: str = None, max_model_len: int = 8192):
        """
        Initialize EmbeddingManager with vLLM server configuration.
        
        Args:
            vllm_base_url: Base URL for the vLLM embedding server (defaults to YAML config)
    ...
    def get_vllm_model_name(self) -> str:
        ...
        try:
            ...
            # Use consistent collection name for keywords
...
# Embedding generation functionality has been integrated into the unified CLI (cli.py)
# Use: python cli.py embed --help


def generate_keywords_embeddings(keywords_file: str, vllm_url: str, max_keywords: Optional[int] = None, force_recreate: bool = False):
    """Generate embeddings for keywords - wrapper function for CLI."""
    
    # Use YAML config defaults if not provided
    vllm_config = CONFIG.get_vllm_config()
    actual_vllm_url = vllm_url or vllm_config['base_url']
    
    embedding_manager = EmbeddingManager(vllm_base_url=actual_vllm_url)
...
def test_embedding_generation(text: str, vllm_url: str):
    """Test embedding generation - wrapper function for CLI."""
    from openai import OpenAI
    from chromadb.utils.embedding_functions import OpenAIEmbeddingFunction
